...
func TestReadmeDocumentsRunScripts(t *testing.T) {
	...
	for _, want := range []string{
		...
		"llmgate update",
		"Run Script Details",
		"install or update the user-local",
		"Install locations",
		"`$HOME/.local/bin/llmgate`",
		"`$env:LOCALAPPDATA\\Programs\\llmgate\\llmgate.exe`",
		"Install metadata locations",
		"do not edit shell profiles or User `PATH`",
